(() => {
  ...
  const privacyVersion = String(form.getAttribute('data-privacy-version') || '').trim();
  ...
  function analyticsAllowed() {
    try {
      return window.localStorage.getItem('s90g_cookie_consent') === 'accepted';
    } catch {
      return false;
    }
  }
  ...
  function track(name, extra = {}) {
    ...
    window.gtag('event', name, {
      event_category: 'professional_interest',
  ...
  function applyInactiveState() {
    ...
    show(
      'Il modulo non è ancora attivo. La pagina è predisposta soltanto per il collaudo tecnico prima della pubblicazione.',
      'info',
    );
  }

  if (!active || !endpoint || !privacyVersion) {
    applyInactiveState();
    return;
  }
  ...
  form.addEventListener('submit', async event => {
    ...
    if (email && !consentReply && !consentUpdates) {
      show('Se indichi l’email, seleziona almeno una finalità autorizzata.', 'error');
      return;
    }
    if (!email && consentReply) {
      show('Indica un’email per ricevere la risposta scritta.', 'error');
      return;
    }
    if (!email && consentUpdates) {
      show('Indica un’email per ricevere eventuali aggiornamenti.', 'error');
      return;
    }
    ...
    try {
      ...
      show(
        'Invio non riuscito. Riprova più tardi oppure usa i contatti indicati nel sito.',
        'error',
      );
