...
const htmlFiles=[];const walk=dir=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){if(entry.name==='.git'||entry.name.startsWith('._'))continue;const full=path.join(dir,entry.name);if(entry.isDirectory())walk(full);else if(entry.name.endsWith('.html'))htmlFiles.push(full)}};walk(path.join(__dirname,'..'));let visual=0,privacy=0;for(const file of htmlFiles){const raw=fs.readFileSync(file,'utf8');if(raw.includes('sistema90g-visual-2026.css')){visual++;assert.ok(raw.includes('sistema90g-visual-2026.css?v=20260730a'),file)}if(raw.includes('privacy-consent.js')){privacy++;assert.ok(raw.includes('privacy-consent.js?v=20260730a'),file)}}assert.ok(visual>=50);assert.ok(privacy>=50);const pathPage=fs.readFileSync(path.join(__dirname,'..','analisi-preventiva.html'),'utf8');assert.ok(pathPage.includes('role-case-path.css?v=20260728e'));assert.ok(pathPage.includes('role-case-path.js?v=20260728e'));const consent=fs.readFileSync(path.join(__dirname,'..','privacy-consent.js'),'utf8');assert.ok(consent.includes('/navigation-conversion.js?v=20260728e'));console.log('SECTION D CONTRACT TEST: PASS');
