...
BRANCH="visual-system-v9-guide-hub-pilot"
PORT="4181"
BASE_URL="http://127.0.0.1:${PORT}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
OUT="$HOME/Desktop/Sistema90G-Guide-Hub-Visual-V1-Audit"
...
python3 -m http.server "$PORT" --bind 127.0.0.1 --directory dist >/tmp/s90g-guide-hub-v1-audit-preview.log 2>&1 &
server_pid=$!
...
node "$ROOT/tools/capture_visual_page.mjs" "$CHROME" "$OUT" "$BASE_URL" "preventivo-acquisto-cucina-guide.html" "guide-hub-preventivo"

cat > "$OUT/README.txt" <<EOF
Sistema 90G — Guide Hub Visual V1 audit locale
Branch: $BRANCH
Pagina: $BASE_URL/preventivo-acquisto-cucina-guide.html
...
File:
- guide-hub-preventivo-full-desktop.png
- guide-hub-preventivo-full-mobile.png
- metrics.json
EOF
...
echo
echo "GUIDE HUB VISUAL V1: GATE LOCALE COMPLETATO."
echo "Risultati: $OUT"
